...
// HyperVConfigListLoader.java --
//
// HyperVConfigListLoader.java is part of ElectricCommander.
//
...
public class HyperVConfigListLoader
    extends Loader
{

    //~ Instance fields --------------------------------------------------------

    private final HyperVConfigList m_configList;
    private final CgiRequestProxy  m_cgiRequestProxy;
    private String                 m_editorName;

    //~ Constructors -----------------------------------------------------------

    public HyperVConfigListLoader(
            HyperVConfigList configList,
            Component        component,
            ...
            ChainedCallback  callback)
    {
        ...
        m_cgiRequestProxy = new CgiRequestProxy(getPluginName(), "hyperv.cgi");
    }
    ...
    private void loadConfigs(Map<String, String> cgiParams)
    {
        ...
        try {
                    ...
                    new RequestCallback() {
                                ...
                                Throwable exception)
                        {
                            ((HasErrorPanel) m_component).addErrorMessage(
                                "Error loading MSSystemCenterVMM configuration list: ",
                                exception);
                        }
            ...
            if (m_component instanceof HasErrorPanel) {
                ((HasErrorPanel) m_component).addErrorMessage(
                    "Error loading MSSystemCenterVMM configuration list: ", e);
            }
            else {
                m_component.getLog()
                           .error(e);
    ...
    private void loadEditors()
    {
        ...
        request.setPropertyName(
            "/plugins/EC-MSSystemCenterVMM/project/ui_forms/"
                + m_editorName);
        request.setExpand(false);
        request.setCallback(new EditorLoaderCallback("hypervcfg"));
        ...
        implements PropertyCallback
    {
        ...
        @Override public void handleResponse(Property response)
        {
            ...
            String errorMsg = "Editor '" + m_editorName
                    + "' not found for MSSystemCenterVMM plugin '"
                    + m_configPlugin + "'";
