...
# hyperv.cgi -
#
# Get/set EC-MSSystemCenterVMM configuration info for UI
#
...
################################
# getCfgList - Return the list of configurations from MSSystemCenterVMM
#
# Arguments:
#   none
#
...
sub getCfgList {
    ...
    my $gcfg = new ElectricCommander::PropDB($::ec,"/projects/@PLUGIN_NAME@/hyperv_cfgs");
    ...
    foreach my $cfg (keys  %cfgs) {
        my $host = $gcfg->getCol("$cfg/hyperv_host");

        $xml .= "  <cfg>\n";
        $xml .= "     <name>$cfg</name>\n";
        $xml .= "     <host>" . xmlQuote($host) . "</host>\n";
        $xml .= "  </cfg>\n";
    }
    $xml .= "</cfgs>\n";
